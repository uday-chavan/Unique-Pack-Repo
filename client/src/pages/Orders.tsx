...
import {
  ...
  AlertCircle, Trash2, PackageCheck, Download, MoreHorizontal, Pencil, Search, Filter
} from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
...
export default function Orders() {
      ...
      <div style="border: 2px solid black; padding: 24px; font-family: Arial, sans-serif; color: black;">
        ...
        <div style="margin-bottom: 24px;">
          ...
          <table style="width: 100%; font-size: 12px; border-collapse: collapse;">
            <tbody>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">GSTIN of Supplier</td><td style="padding: 8px;">27AGJPJ6286A1ZD, UNIQ PACK</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">Place of Dispatch</td><td style="padding: 8px;">${details.placeOfDispatch || "Shrirampur, MAHARASHTRA-423603"}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">GSTIN of Recipient</td><td style="padding: 8px;">${order?.customer?.gstin || order?.customer?.taxId || "N/A"}, ${order?.customer?.businessName || order?.customer?.name}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">Place of Delivery</td><td style="padding: 8px;">${details.toLocation || order?.customer?.address || "N/A"}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">Document No.</td><td style="padding: 8px;">${details.documentNo || `UP/2025-26/${order?.id?.toString().padStart(4, "0")}`}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">Document Date</td><td style="padding: 8px;">${createdDate.toLocaleDateString('en-IN')}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">Transaction Type</td><td style="padding: 8px;">${details.transactionType || "Regular"}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">Value of Goods</td><td style="padding: 8px;">₹${Number(order?.totalAmount || 0).toLocaleString("en-IN")}</td></tr>
              <tr style="border-bottom: 1px solid #ccc;"><td style="font-weight: bold; width: 160px; padding: 8px;">HSN Code</td><td style="padding: 8px;">${details.hsnCode || "8422"} - PACKING MACHINE</td></tr>
              <tr><td style="font-weight: bold; width: 160px; padding: 8px;">Reason for Transportation</td><td style="padding: 8px;">${details.transportationReason || "Outward - Supply"}</td></tr>
            </tbody>
          </table>
        </div>
        <div style="margin-bottom: 24px;">
          ...
          <table style="width: 100%; font-size: 12px; border-collapse: collapse; border: 1px solid #999;">
            <thead><tr style="background-color: #f0f0f0;"><th style="border: 1px solid #999; padding: 8px; text-align: left;">Vehicle / Trans Mode</th><th style="border: 1px solid #999; padding: 8px; text-align: left;">From</th><th style="border: 1px solid #999; padding: 8px; text-align: left;">Entered Date</th><th style="border: 1px solid #999; padding: 8px; text-align: left;">Entered By</th><th style="border: 1px solid #999; padding: 8px; text-align: left;">CEWB No.</th><th style="border: 1px solid #999; padding: 8px; text-align: left;">Portal</th></tr></thead>
            <tbody><tr style="border: 1px solid #999;"><td style="border: 1px solid #999; padding: 8px;">${details.transportMode || "Road"} - ${details.vehicleNo || "MH09CU6678"}</td><td style="border: 1px solid #999; padding: 8px;">${details.fromLocation || "Shrirampur"}</td><td style="border: 1px solid #999; padding: 8px;">${createdDate.toLocaleDateString('en-IN')} ${createdDate.toLocaleTimeString('en-IN')}</td><td style="border: 1px solid #999; padding: 8px;">${details.enteredBy || "27AGJPJ6286A1ZD"}</td><td style="border: 1px solid #999; padding: 8px;">-</td><td style="border: 1px solid #999; padding: 8px;">1</td></tr></tbody>
          </table>
        </div>
  ...
  const handlePayment = async () => {
    ...
    await updatePayment.mutateAsync({ orderId: paymentOrder.id, amountPaid: paymentAmount });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "paid": return <Badge className="bg-emerald-500 hover:bg-emerald-600"><CheckCircle className="w-3 h-3 mr-1" /> Paid</Badge>;
      case "partial": return <Badge variant="outline" className="text-blue-500 border-blue-200 bg-blue-50"><AlertCircle className="w-3 h-3 mr-1" /> Partial</Badge>;
      case "pending": return <Badge variant="outline" className="text-amber-500 border-amber-200 bg-amber-50"><Clock className="w-3 h-3 mr-1" /> Pending</Badge>;
  ...
  const getDeliveryBadge = (status: string) => {
    switch (status?.toLowerCase()) {
      case "delivered": return <Badge className="bg-emerald-500 hover:bg-emerald-600"><PackageCheck className="w-3 h-3 mr-1" /> Delivered</Badge>;
      case "pending": return <Badge variant="outline" className="text-amber-500 border-amber-200 bg-amber-50"><Truck className="w-3 h-3 mr-1" /> Pending</Badge>;
      case "in-transit": return <Badge variant="outline" className="text-blue-500 border-blue-200 bg-blue-50"><Truck className="w-3 h-3 mr-1" /> In Transit</Badge>;
      default: return <Badge variant="secondary"><Truck className="w-3 h-3 mr-1" /> {status || "Pending"}</Badge>;
    }
  };
  ...
  return (
    <Shell>
                ...
                <>
                  {[...Array(6)].map((_, idx) => (
                    <TableRow key={`skeleton-${idx}`} className="border-b border-black">
                      ...
                      <TableCell className="py-4"><Skeleton className="h-6 w-20" /></TableCell>
                      <TableCell className="py-4"><Skeleton className="h-8 w-8" /></TableCell>
                    </TableRow>
                  ))}
                </>
                      ...
                      <div className="flex flex-col gap-1">
                        {order.items?.map((item: any, i: number) => (
                          <div key={i} className="flex items-start gap-1 min-w-0">
                            ...
                            <span className="truncate" title={item.machine?.name}>{item.machine?.name}</span>
                          </div>
                        ))}
                    ...
                    <TableCell className="transition-transform duration-300 group-hover:translate-x-1">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          <DropdownMenuItem onClick={() => openPaymentDialog(order)} disabled={order.paymentStatus === "paid"} data-testid={`button-payment-${order.id}`} className="cursor-pointer">
                          ...
                          {order.deliveryStatus?.toLowerCase() !== "delivered" && (
                            <DropdownMenuItem onClick={() => handleDeliveryStatusUpdate(order.id, "delivered")} className="cursor-pointer text-emerald-600 focus:text-emerald-600" data-testid={`button-deliver-${order.id}`}>
                              <PackageCheck className="w-4 h-4 mr-2" /> Mark Delivered
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuItem onClick={() => openInvoiceDialog(order)} className="cursor-pointer text-blue-600 focus:text-blue-600">
                            <FileText className="w-4 h-4 mr-2" /> View Invoice
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => openEWayBillDialog(order)} className="cursor-pointer text-green-600 focus:text-green-600">
                            <FileText className="w-4 h-4 mr-2" /> View e-Way Bill
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => setOrderToDelete(order)} className="cursor-pointer text-red-600 focus:text-red-600" data-testid={`button-delete-${order.id}`}>
      ...
      <AlertDialog open={!!orderToDelete} onOpenChange={(open) => !open && setOrderToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure you want to delete Order #{orderToDelete?.id?.toString().padStart(5, "0")}? This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setOrderToDelete(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { deleteOrder.mutateAsync(orderToDelete.id); setOrderToDelete(null); }} className="bg-red-600 hover:bg-red-700">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={!!paymentOrder} onOpenChange={(open) => !open && setPaymentOrder(null)}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            ...
            <DialogDescription>Enter the amount paid for Order #{paymentOrder?.id?.toString().padStart(5, "0")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Total Amount:</span><span className="font-bold">₹{Number(paymentOrder?.totalAmount || 0).toLocaleString("en-IN")}</span></div>
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Previously Paid:</span><span>₹{Number(paymentOrder?.amountPaid || 0).toLocaleString("en-IN")}</span></div>
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Balance Due:</span><span className="text-red-600 font-medium">₹{(Number(paymentOrder?.totalAmount || 0) - Number(paymentOrder?.amountPaid || 0)).toLocaleString("en-IN")}</span></div>
            <div className="space-y-2">
              <Label htmlFor="paymentAmount">Amount to Pay (₹)</Label>
              <Input id="paymentAmount" type="number" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} placeholder="Enter amount" data-testid="input-payment-amount" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setPaymentOrder(null)}>Cancel</Button>
            <Button onClick={handlePayment} disabled={updatePayment.isPending || !paymentAmount} className="bg-emerald-600 hover:bg-emerald-700" data-testid="button-confirm-payment">
      ...
      {/* e-Way Bill Dialog */}
      <Dialog open={!!selectedOrderForEWayBill} onOpenChange={(open) => { if (!open) { setSelectedOrderForEWayBill(null); setIsEWayBillEditMode(false); setEWayBillDetails({}); } }}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto overflow-x-hidden">
          <div className="flex items-center justify-between">
            <div><DialogTitle>e-Way Bill Preview</DialogTitle><DialogDescription>Preview and download e-Way bill.</DialogDescription></div>
          ...
          {isEWayBillEditMode && (
            <div className="grid grid-cols-2 gap-4 text-xs mb-4 max-h-80 overflow-y-auto border border-black rounded p-4 bg-slate-50">
              ...
              <div><Label>HSN Code</Label><Input value={eWayBillDetails.hsnCode || ""} onChange={(e) => handleEWayBillDetailsChange("hsnCode", e.target.value)} placeholder="HSN Code" /></div>
              <div><Label>Place of Dispatch</Label><Input value={eWayBillDetails.placeOfDispatch || ""} onChange={(e) => handleEWayBillDetailsChange("placeOfDispatch", e.target.value)} placeholder="Place of Dispatch" /></div>
              <div><Label>Document No</Label><Input value={eWayBillDetails.documentNo || ""} onChange={(e) => handleEWayBillDetailsChange("documentNo", e.target.value)} placeholder="Document Number" /></div>
              <div><Label>Transaction Type</Label><Input value={eWayBillDetails.transactionType || ""} onChange={(e) => handleEWayBillDetailsChange("transactionType", e.target.value)} placeholder="Regular" /></div>
              <div><Label>Reason for Transportation</Label><Input value={eWayBillDetails.transportationReason || ""} onChange={(e) => handleEWayBillDetailsChange("transportationReason", e.target.value)} placeholder="Outward - Supply" /></div>
              <div><Label>From Location</Label><Input value={eWayBillDetails.fromLocation || ""} onChange={(e) => handleEWayBillDetailsChange("fromLocation", e.target.value)} placeholder="From Location" /></div>
              <div><Label>Entered By (GSTIN)</Label><Input value={eWayBillDetails.enteredBy || ""} onChange={(e) => handleEWayBillDetailsChange("enteredBy", e.target.value)} placeholder="GSTIN" /></div>
              <div><Label>Enter Place to Deliver</Label><Input value={eWayBillDetails.toLocation || ""} onChange={(e) => handleEWayBillDetailsChange("toLocation", e.target.value)} placeholder="Place of Delivery" /></div>
            </div>
          )}
          ...
          <DialogFooter>
            {isEWayBillEditMode ? (
              <>
                <Button variant="outline" onClick={() => { setIsEWayBillEditMode(false); setEWayBillDetails({}); }}>Cancel</Button>
                <Button onClick={handleSaveEWayBillDetails} disabled={updateOrderDetails.isPending} className="bg-emerald-600 hover:bg-emerald-700">{updateOrderDetails.isPending ? "Saving..." : "Save Details"}</Button>
              </>
            ) : (
              <>
                <Button variant="outline" onClick={() => { setSelectedOrderForEWayBill(null); setIsEWayBillEditMode(false); setEWayBillDetails({}); }}>Close</Button>
                <Button onClick={downloadEWayBill} className="bg-green-600 hover:bg-green-700"><Download className="w-4 h-4 mr-2" /> Download PDF</Button>
              </>
            )}
          </DialogFooter>
        </DialogContent>
      ...
      {/* Invoice Dialog */}
      <Dialog open={!!selectedOrderForInvoice} onOpenChange={(open) => { if (!open) { setSelectedOrderForInvoice(null); setInvoiceDetails({}); setIsInvoiceEditMode(false); } }}>
        <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
          <div className="flex items-center justify-between">
            <div><DialogTitle>Tax Invoice</DialogTitle><DialogDescription>View and download invoice</DialogDescription></div>
          ...
          {isInvoiceEditMode && (
            <div className="grid grid-cols-2 gap-4 text-xs mb-4 max-h-64 overflow-y-auto border border-black rounded p-4 bg-slate-50">
              ...
              <div><Label>E-Way Bill No</Label><Input value={invoiceDetails.eWayBillNo || ""} onChange={(e) => handleInvoiceDetailsChange("eWayBillNo", e.target.value)} placeholder="E-Way Bill No" /></div>
              <div><Label>Discount %</Label><Input type="number" value={invoiceDetails.discountPercent || ""} onChange={(e) => handleInvoiceDetailsChange("discountPercent", e.target.value)} placeholder="0" /></div>
              <div><Label>Bank Name</Label><Input value={invoiceDetails.bankName || ""} onChange={(e) => handleInvoiceDetailsChange("bankName", e.target.value)} placeholder="Bank of Baroda" /></div>
              <div><Label>Bank Branch</Label><Input value={invoiceDetails.bankBranch || ""} onChange={(e) => handleInvoiceDetailsChange("bankBranch", e.target.value)} placeholder="Kopargaon" /></div>
              <div><Label>Account No</Label><Input value={invoiceDetails.accountNo || ""} onChange={(e) => handleInvoiceDetailsChange("accountNo", e.target.value)} placeholder="Account Number" /></div>
              <div><Label>IFSC Code</Label><Input value={invoiceDetails.ifscCode || ""} onChange={(e) => handleInvoiceDetailsChange("ifscCode", e.target.value)} placeholder="IFSC Code" /></div>
              <div><Label>Mode of Transport</Label><Input value={invoiceDetails.modeOfTransport || ""} onChange={(e) => handleInvoiceDetailsChange("modeOfTransport", e.target.value)} placeholder="Road" /></div>
              <div><Label>Dispatched From</Label><Input value={invoiceDetails.dispatchedFrom || ""} onChange={(e) => handleInvoiceDetailsChange("dispatchedFrom", e.target.value)} placeholder="Kopargaon" /></div>
              <div><Label>Place of Supply</Label><Input value={invoiceDetails.placeOfSupply || ""} onChange={(e) => handleInvoiceDetailsChange("placeOfSupply", e.target.value)} placeholder="Kopargaon" /></div>
            </div>
          )}
              ...
              <div className="border-2 border-black p-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-[9px] border-t-2 border-black pt-2">
                  ...
                  <div className="border-2 border-black">
                    <div className="flex justify-between p-1 border-b border-black"><span>Sub Total</span><span>₹{Number(selectedOrderForInvoice?.totalAmount || 0).toLocaleString("en-IN")}</span></div>
                    <div className="flex justify-between p-1 border-b border-black"><span>Discount @{invoiceDetails.discountPercent || "0"}%</span><span>₹0</span></div>
                    <div className="flex justify-between p-1 border-b border-black"><span>CGST @ 9%</span><span>₹{(Number(selectedOrderForInvoice?.totalAmount || 0) * 0.09).toLocaleString("en-IN")}</span></div>
                    <div className="flex justify-between p-1 border-b border-black"><span>SGST @ 9%</span><span>₹{(Number(selectedOrderForInvoice?.totalAmount || 0) * 0.09).toLocaleString("en-IN")}</span></div>
                    <div className="flex justify-between p-1 font-bold bg-gray-100"><span>TOTAL AMOUNT</span><span>₹{(Number(selectedOrderForInvoice?.totalAmount || 0) * 1.18).toLocaleString("en-IN")}</span></div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-end mt-6 text-[9px]">
                  <div className="text-center"><div className="border-t border-black w-32"></div><p>Customer Signature</p></div>
                  <div className="text-right"><p><strong>For UNIQ PACK</strong></p><div className="h-8"></div><p>Authorized Signatory</p></div>
                </div>
              </div>
            </div>
          </div>
          <DialogFooter>
            {isInvoiceEditMode ? (
              <>
                <Button variant="outline" onClick={() => { setIsInvoiceEditMode(false); setInvoiceDetails({}); }}>Cancel</Button>
                <Button onClick={handleSaveInvoiceDetails} disabled={updateOrderDetails.isPending} className="bg-emerald-600 hover:bg-emerald-700">{updateOrderDetails.isPending ? "Saving..." : "Save Details"}</Button>
              </>
            ) : (
              <>
                <Button variant="outline" onClick={() => { setSelectedOrderForInvoice(null); setInvoiceDetails({}); setIsInvoiceEditMode(false); }}>Close</Button>
                <Button onClick={downloadInvoice} className="bg-blue-600 hover:bg-blue-700"><Download className="w-4 h-4 mr-2" /> Download PDF</Button>
              </>
            )}
          </DialogFooter>
        </DialogContent>
