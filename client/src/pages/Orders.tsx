...
export default function Orders() {
  ...
  const openInvoiceDialog = (order: any) => {
    setInvoiceDetails({
      ...
      cgstPercent: order.cgstPercent || "9.0",
      sgstPercent: order.sgstPercent || "9.0",
        ...
        <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
          ...
          {isInvoiceEditMode && (
            <div className="grid grid-cols-2 gap-4 text-xs mb-4 max-h-64 overflow-y-auto border border-black rounded p-4 bg-slate-50">
              ...
              <div><Label>CGST %</Label><Input type="number" step="0.5" value={invoiceDetails.cgstPercent || ""} onChange={(e) => handleInvoiceDetailsChange("cgstPercent", e.target.value)} placeholder="9.0" /></div>
              <div><Label>SGST %</Label><Input type="number" step="0.5" value={invoiceDetails.sgstPercent || ""} onChange={(e) => handleInvoiceDetailsChange("sgstPercent", e.target.value)} placeholder="9.0" /></div>
              ...
              <div className="border-2 border-black p-3">
                <div className="flex justify-between items-start border-b-2 border-black pb-3 mb-3">
                  ...
                  <div className="text-right text-[9px]">
                    ...
                    <p><strong>Company's Import Export Code:</strong> AGJPJ6286A</p>
                ...
                <div className="grid grid-cols-2 gap-2 text-[9px] border-t-2 border-black pt-2">
                  ...
                  <div className="border-2 border-black">
                    ...
                    <div className="flex justify-between p-1 border-b border-black"><span>CGST @ {invoiceDetails.cgstPercent || selectedOrderForInvoice?.cgstPercent || "9"}%</span><span>₹{(Number(selectedOrderForInvoice?.totalAmount || 0) * (Number(invoiceDetails.cgstPercent || selectedOrderForInvoice?.cgstPercent || 9) / 100)).toLocaleString("en-IN")}</span></div>
                    <div className="flex justify-between p-1 border-b border-black"><span>SGST @ {invoiceDetails.sgstPercent || selectedOrderForInvoice?.sgstPercent || "9"}%</span><span>₹{(Number(selectedOrderForInvoice?.totalAmount || 0) * (Number(invoiceDetails.sgstPercent || selectedOrderForInvoice?.sgstPercent || 9) / 100)).toLocaleString("en-IN")}</span></div>
                    <div className="flex justify-between p-1 font-bold bg-gray-100"><span>TOTAL AMOUNT</span><span>₹{(Number(selectedOrderForInvoice?.totalAmount || 0) * (1 + (Number(invoiceDetails.cgstPercent || selectedOrderForInvoice?.cgstPercent || 9) + Number(invoiceDetails.sgstPercent || selectedOrderForInvoice?.sgstPercent || 9)) / 100)).toLocaleString("en-IN")}</span></div>
                  </div>
                </div>
